...
export const Add_account = async(request,response) => {
    ...
    try{
        ...
        let user1 = await User.findByIdAndUpdate(userData,{$push: { Accounts: newAccount._id }}).populate('Accounts').populate({
            path: 'Transactions',
            populate: { 
              path: 'Account', 
            }
...
export const Add_transaction = async (request,response) => {
    try{
       ...
       let udpateaccount = await Account.findByIdAndUpdate(request.body.Account,{$inc: { balance: Amount1 }},{ new: true } );

       let user = await User.findByIdAndUpdate(userData,{$push: { Transactions: newtransaction._id }},{new: true}).populate('Accounts').populate({
        path: 'Transactions',
        populate: { 
          path: 'Account', 
        }
